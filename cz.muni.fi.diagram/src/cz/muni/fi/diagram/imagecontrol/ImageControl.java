package cz.muni.fi.diagram.imagecontrol;
...
/**
 * The control for viewing the generated images
 */
public class ImageControl extends Canvas {
	...
	public ImageControl(final Composite parent, int style) {
		...
		addPaintListener(event -> paint(event.gc)); // paint listener
				/*new PaintListener() { 
	...
	private class PanHandler implements MouseListener, MouseMoveListener {
		...
		public void mouseMove(MouseEvent e) {
			if (panPoint != null && panDelta != null) {
				...
				String toolTip = (link != null ? link.toString() : null);
				String oldToolTip = getToolTipText();
				if (!Objects.equals(toolTip, oldToolTip) && toolTip == null) {
	...
	private Collection<ILinkSupport> linkSupports = new ArrayList<>();
